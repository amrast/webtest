...
def jsonify(data):
    ...
    try:
        ...
        json_file = "file.json"
        try :
            json_data["array"] = json.dump(data.tolist(), codecs.open(json_file, 'w', encoding='utf-8'))
        except Exception:
            json_data["array"] = json.dump(data, codecs.open(json_file, 'w', encoding='utf-8'))


    return json.dumps(json_data)


class Word_In_Vocab(Resource):
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument('word', type=str, required=True, help="Word cannot be blank!")
        args = parser.parse_args()
        try:
            res = in_vocab(args['word'])
            res = base64.b64encode(str(res))
            return res
        except Exception, e:
            print e
            return
...
class ModelWordSet(Resource):
    def get(self):
        try:
            vocab = model.vocab
            res = base64.b64encode(str(set(vocab)))
            return res
        except Exception, e:
            print e
            return
...
if __name__ == '__main__':
    ...
    print "Loading WordVectorModel" + model_path
    ...
    api.add_resource(Word_In_Vocab, '/word_check')
